...
void rasterizer_draw_rect(int left, int top, int right, int bottom)
{
	...
	if (
		(left < 0) || (right >= pcontext->screen_texture_pixels_wide) ||
		(top < 0) || (bottom >= pcontext->screen_texture_pixels_high) 
		)
	{
		return;
	}
	...
	unsigned char* src_pixels)
{
	...
	p5x = swr_get_max(p1x, p3x);
	p5y = swr_get_max(p1y, p3y);

	p6x = swr_get_min(p2x, p4x);
	p6y = swr_get_min(p2y, p4y);
	/*clip new */
	sx = p5x - p1x;
	sy = p5y - p1y;
	tw = abs(p5x - p6x);
